...
class KssSection {
  ...
  toJSON() {
    ...
    returnObject.childSections = this.childSections().map(childSection => {
      return childSection.toJSON();
    });
  ...
  /**
   * Gets or sets the child sections of a section.
   *
   * If the `childSections` value is provided, the `childSections` for this section is set.
   * Otherwise, the `childSections` of the section is returned.
   *
   * @param {array} [childSections] Optional. An array of other `KssSections` objects.
   * @returns {KssSection|array} If `childSections` is given, the `KssSection` object
   *   is returned to allow chaining of methods. Otherwise, the `childSections` of the
   *   section is returned.
   */
  childSections(childSections) {
    if (typeof childSections === 'undefined') {
      return this.data.childSections;
    }

    this.data.childSections.concat(childSections);
